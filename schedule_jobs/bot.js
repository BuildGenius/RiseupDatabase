...
async function check_shedule_jobs () {
    let query = `USE msdb ; 
...
async function check_shedule_jobs_dataraw () {
    let query = `USE msdb ; 
        DECLARE @now INT = FORMAT(DATEADD(DAY, -1, GETDATE()), 'yyyyMMdd');
 ...
 async function chk_temp_and_rep () {
    let autobot = new bot;
...
cron.schedule('0 9 * * * ', async () => {
    await chk_temp_and_rep();
});

cron.schedule('30 11 * * *', async () => {
    await check_shedule_jobs();
    await check_shedule_jobs_dataraw();
});